...
try {
    ...
    # Delete stale cache files to prevent "source directory mismatch" errors
    $CacheFile = Join-Path $BuildDir "CMakeCache.txt"
    if (Test-Path $CacheFile) {
        Write-Host "[INFO] Deleting stale CMakeCache.txt: $CacheFile"
        Remove-Item $CacheFile -Force
    }
    $CMakeFiles = Join-Path $BuildDir "CMakeFiles"
    if (Test-Path $CMakeFiles) {
        Write-Host "[INFO] Deleting stale CMakeFiles folder: $CMakeFiles"
