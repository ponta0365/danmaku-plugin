...
class YouTubeAdapter:
    ...
    def _fetch_live_chat_id(self) -> Optional[str]:
        ...
        try:
            logger.info("Searching for active YouTube broadcasts...")
            ...
            if res.status_code == 200:
                ...
                if items:
                    ...
                    if chat_id:
                        logger.info("Found active YouTube broadcast chat ID.")
                        return chat_id
            else:
                logger.error(f"YouTube broadcasts active lookup HTTP {res.status_code}: {res.text}")
        except Exception as e:
            logger.error(f"API request exception during active broadcast lookup: {e}")
            
        # 2. Fallback to upcoming/testing broadcasts (useful during preview state or scheduled streams)
        try:
            logger.info("Searching for upcoming/scheduled YouTube broadcasts...")
            url = "https://www.googleapis.com/youtube/v3/liveBroadcasts?broadcastStatus=upcoming&broadcastType=all&part=snippet"
            res = requests.get(url, headers=headers, timeout=10)
            if res.status_code == 200:
                data = res.json()
                items = data.get("items", [])
                if items:
                    # Sort upcoming broadcasts by scheduledStartTime to get the nearest one
                    items_sorted = sorted(
                        items,
                        key=lambda x: x.get("snippet", {}).get("scheduledStartTime", "")
                    )
                    chat_id = items_sorted[0].get("snippet", {}).get("liveChatId")
                    if chat_id:
                        logger.info("Found upcoming/scheduled YouTube broadcast chat ID.")
                        return chat_id
            else:
                logger.error(f"YouTube broadcasts upcoming lookup HTTP {res.status_code}: {res.text}")
        except Exception as e:
            logger.error(f"API request exception during upcoming broadcast lookup: {e}")
