                                       ...
                                       int& out_width, int& out_height)
{
    ...
    RectF bound_rect;
    g_measure.MeasureString(text.c_str(), -1, &font, layout_rect, &format, &bound_rect);
    ReleaseDC(NULL, hdc);
    
    float outline_thickness = font_size * (outline_size_percent / 100.0f);
    if (outline_thickness < 1.0f) outline_thickness = 1.0f;
    
    int width = (int)ceil(bound_rect.Width) + (int)ceil(outline_thickness * 2.0f) + 4;
    int height = (int)ceil(bound_rect.Height) + (int)ceil(outline_thickness * 2.0f) + 4;
    ...
    GraphicsPath path;
    path.AddString(text.c_str(), -1, &family, FontStyleBold, font_size, PointF(outline_thickness + 2.f, outline_thickness + 2.f), &format);
...
static void PushComment(struct danmaku_source *context, const std::wstring& text)
{
    ...
    gs_texture_t *tex = CreateTextTexture(text, context->font_name, (float)context->font_size,
                                           context->text_color, context->outline_color,
...
static void danmaku_video_tick(void *data, float seconds)
{
    ...
    if (now_ms - context->last_push_time_ms >= (uint64_t)context->min_push_interval_ms) {
        ...
        if (context->nigayaka_prob > 0 && (rand() % 100) < context->nigayaka_prob) {
